<template>
  <el-card class="box-card">
    ...
    <div class="table-box" v-if="showTable">
      <el-table :data="tableData" style="width: 100%;text-align: center" :cell-style="{ textAlign: 'center' }"
...
import { getUserAuthApi, deleteAuthApi, getApolloApi } from "../../api/userAuth/userAuth"
...
const confirmDelete = (row) => {
  ElMessageBox.confirm(
    ...
    const { data } = await deleteAuthApi(params)
    if (data.code === 200) {
      ElMessage({
        type: 'success',
        message: '删除成功!',
...
const { userId, tableData, showTable, pageIndex, pageSize, total, loading } = toRefs(state)
</script>
